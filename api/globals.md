...
* module
* require()
以下的对象是特定于 AutoTools 的。 有些内置对象是 JavaScript 语言本身的一部分，它们也是全局的。
...
* `version` {string} | {number} AutoTools的版本或版本号

表示此脚本需要AutoTools版本达到指定版本才能运行。例如`requiresAutojsVersion("3.0.0 Beta")`表示脚本需要在AutoTools 3.0.0 Beta以及以上运行。

调用该函数时会判断运行脚本的AutoTools的版本号，如果没有达到要求则抛出异常。

version参数可以是整数表示版本号，例如`requiresAutojsVersion(250)`；也可以是字符串格式表示的版本，例如"3.0.0 Beta", "3.1.0 Alpha4", "3.2.0"等。

可以通过`app.autojs.versionCode`和`app.autojs.versionName`获取当前的AutoTools版本号和版本。
...
尽管安卓有很多权限，但必须写入Manifest才能动态申请，为了防止权限的滥用，目前AutoTools只能额外申请两个权限：
* `access_fine_location` GPS权限
* `record_audio` 录音权限

您可以通过APK编辑器来增加AutoTool以及AutoTool打包的应用的权限。
