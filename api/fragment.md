...
fragment模块提供了编写用户界面的支持。

    给Android开发者或者高阶用户的提醒，AutoTools的fragment系统来自于Android ，所有属性和方法都能在Android源码中找到。如果某些代码或属性没有出现在AutoTools的文档中，可以参考Android的文档。
...
字符串"fragment"的前面可以有注释、空行和空格，但是不能有其他代码。
...
我们使用xml来编写界面，并通过`fragment.layout()`函数指定界面的布局xml。举个例子：
...
fragment.post(() => {
    // 修改文本
   fragment.example.attr("text", "Hello, AutoTools UI");
    // 修改背景
    fragment.example.attr("bg", "#ff00ff");
    // 修改高度
   fragment.example.attr("h", "500dp");
}, 5000);
```

**注意：**并不是所有属性都能在js代码设置，有一些属性只能在布局创建时设置，例如style属性；还有一些属性虽然能在代码中设置，但是还没支持；对于这些情况，在AutoTools Pro 8.1.0+会抛出异常，其他版本则不会抛出异常。
...
如果使用图片地址或本地路径，AutoTools会自动使用适当的缓存来储存这些图片，减少下次加载的时间。
...
例如，圆角矩形的AutoTools图标：`<img w="100" h="100" radius="20" bg="white" src="http://www.autojs.org/assets/uploads/profile/3-profileavatar.png" />`
...
例如, 圆角矩形带灰色边框的AutoTools图标：`<img w="100" h="100" radius="20" borderWidth="5" borderColor="gray" bg="white" src="http://www.autojs.org/assets/uploads/profile/3-profileavatar.png" />`
...
例如，圆形的AutoTools图标：`<img w="100" h="100" circle="true" bg="white" src="http://www.autojs.org/assets/uploads/profile/3-profileavatar.png" />`
...
## fragment.layoutFile(xmlFile)
* `xml` {string} 布局XML文件的路径

此函数和`ui.layout`相似，只不过允许传入一个xml文件路径来渲染布局。

## fragment.inflate(xml[, parent = null, attachToParent = false])
...
for (let i = 0; i < 3; i++) {
    ...
    fragment.container.addView(textView);
}
```
...
log(fragment.isUiThread()); // => true

threads.start(function () {
    log(fragment.isUiThread()); // => false
});

```

## fragment.findView(id)
* `id` {string} View的ID
* 返回 {View}
...
## fragment.post(callback[, delay = 0])
...
```javascript
"fragment";
fragment.statusBarColor("#000000");
```
